...
class GameController:
    ...
    def _fast_execute(self, commands: list[ParsedCommand], state: GameState) -> float:
        ...
        for cmd in commands:
            ...
            logger.debug(f"zoom {direction} at ({cx},{cy})")

        # ③ 再逐条执行move (慢, 每轮1个)
        for cmd in commands:
            if cmd.action.value not in ("move", "attack"):
                continue
            if not cmd.unit_ids or not cmd.target_pixel:
                continue
            ...
            try:
                ...
                logger.warning(f"move失败(用select代替): {str(e)[:60]}")

        elapsed = (time.time() - t0) * 1000
        if select_cmds:
            logger.debug(f"执行: {len(select_cmds)}select + {sum(1 for c in commands if c.action and c.action.value in ('move','attack'))}move ({elapsed:.0f}ms)")
        return elapsed
